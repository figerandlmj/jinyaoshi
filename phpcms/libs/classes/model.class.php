...
class model {
	...
	final public function select_plan($where = '', $data = '*', $limit = '', $order = '', $group = '', $key='') {
		if (is_array($where)) $where = $this->sqls($where);
		return $this->db->select($data, "jys_plan", $where, $limit, $order, $group, $key);
	}

	final public function select_city($where = '', $data = '*', $limit = '', $order = '', $group = '', $key='') {
		if (is_array($where)) $where = $this->sqls($where);
		return $this->db->select($data, "jys_city", $where, $limit, $order, $group, $key);
	}
	final public function select_style($where = '', $data = '*', $limit = '', $order = '', $group = '', $key='') {
		if (is_array($where)) $where = $this->sqls($where);
		return $this->db->select($data, "jys_style", $where, $limit, $order, $group, $key);
	}
	...
	final public function get_name($where = '', $data = '*', $order = '', $group = '') {
		if (is_array($where)) $where = $this->sqls($where);
		return $this->db->get_one($data, "jys_plan", $where, $order, $group);
	}

	final public function get_one_data($where = '', $data = '*', $order = '', $group = '',$tablename) {
		if (is_array($where)) $where = $this->sqls($where);
		return $this->db->get_one($data, $tablename, $where, $order, $group);
	}
	...
	final public function insert_plan($data, $return_insert_id = false, $replace = false) {
		return $this->db->insert($data, "jys_plan", $return_insert_id, $replace);
	}
	final public function insert_city($data, $return_insert_id = false, $replace = false) {
		return $this->db->insert($data, "jys_city", $return_insert_id, $replace);
	}
	...
	final public function update_data($data, $where = '',$tablename) {
		if (is_array($where)) $where = $this->sqls($where);
		return $this->db->update($data, $tablename, $where);
	}
	...
	final public function count_num($where = '') {
		$r = $this->get_name($where, "COUNT(*) AS num");
		return $r['num'];
	}
